...
public class NativeReviewActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		surveySize = (int) (UIUtils.getDimenPixelSize(R.dimen.survey_text_size)
				* TextSizeManager.getInstance().getRealScale());
		lowSurveySize = surveySize;
		middleSueveySize = (int) (UIUtils.getDimenPixelSize(R.dimen.survey_middle_text_size)
				* TextSizeManager.getInstance().getRealScale());
		bigSurveySize = (int) (UIUtils.getDimenPixelSize(R.dimen.survey_big_text_size)
				* TextSizeManager.getInstance().getRealScale());
		/**
		 * 初始化问卷字号动态设置完毕
		 */
		...
		sdImages.setOnDrawerOpenListener(new SlidingDrawer.OnDrawerOpenListener()// 开抽屉
		{
			...
			public void onDrawerOpened() {
				...
				while (iter.hasNext()) {
					...
					if ("thumbnail".equals(image.getName().substring(image.getName().lastIndexOf("_") + 1,
		...
		if (Cnt.TYPE_HEADER == q.qType) {// 假如是标题
			...
			ImageGetter imgGetter = new Html.ImageGetter() {
				public Drawable getDrawable(String source) {
					...
					String name = NativeReviewActivity.this.getFilesDir() + File.separator + "survey" + File.separator
			...
			if (!Util.isEmpty(radioRows)) {
				...
				for (int i = 0; i < radioRows.size(); i++) {
					...
					if (1 == item.isOther) {// 其他项
						 ...
						 */
						// 生成一项的整体布局
						LinearLayout rbA = new LinearLayout(NativeReviewActivity.this);
						...
						if (isMultiline) {// 如果是多列则进入判断
							...
							if (0 == lineNum) {// 如果是每行第一列则新创建一个横向布局
								LinearLayout rbM = new LinearLayout(NativeReviewActivity.this);
						...
						if (0 < childRows.size()) {// 是否有子项
							TextView tvItemTitle = new TextView(NativeReviewActivity.this);
							// 追加说明新布局原FILL_WRAP
							...
							ImageGetter imgGetter = new Html.ImageGetter() {
								public Drawable getDrawable(String source) {
									Drawable drawable = null;
									String name = NativeReviewActivity.this.getFilesDir() + File.separator + "survey"
				...
				for (int i = 0; i < otherItems.size(); i++) {
					...
					if (1 != item.isOther) {
						 ...
						 * 生成一个单选布局
						 */
						LinearLayout rbLL = new LinearLayout(NativeReviewActivity.this);
						...
						if (!Util.isEmpty(item.popUp)) {
							 ...
							 * 生成弹窗说明小图
							 */
							RadioButton imTv = new RadioButton(NativeReviewActivity.this);
			...
			if (!Util.isEmpty(checkRows)) {
				...
				for (int i = 0; i < checkRows.size(); i++) {
					...
					if (1 == item.isOther) {
						...
						LinearLayout llcb = new LinearLayout(NativeReviewActivity.this);
						llcb.setOrientation(LinearLayout.HORIZONTAL);
						llcb.setGravity(Gravity.CENTER_VERTICAL);
						TextView tvcb = new TextView(NativeReviewActivity.this);
						...
						if (!Util.isEmpty(item.caption)) {
							if (1 == item.caption_check) {
								...
								ImageGetter imgGetter = new Html.ImageGetter() {
									public Drawable getDrawable(String source) {
										Drawable drawable = null;
										String name = NativeReviewActivity.this.getFilesDir() + File.separator
...
//						}
		...
		case Cnt.TYPE_MATRIX_RADIO_BUTTON:// 矩阵单选
			...
			for (int r = 0; r < rRows.size() + 1; r++) {
				 ...
				 * 遍历每一列
				 */
				RadioGroup rg = new RadioGroup(NativeReviewActivity.this);// 每一列的
				...
				for (int c = 0; c < rColmns.size() + 1 + rightNum; c++) {
					...
					TextView tvTb = new TextView(NativeReviewActivity.this);
					...
					if (0 == r) {// 如过是第一行, 则打印出每一列的值
						...
						if (0 == c) {
							...
							// if (1 == q.qStarCheck) {
							// TextView tvLine = new
							// TextView(NativeReviewActivity.this);
						...
						// 矩阵右侧 新加的
							...
							RadioButton radio = new RadioButton(NativeReviewActivity.this);
							...
							if (q.qStarCheck != 0) {
								...
								case 1:
									...
									TextView tvLine = new TextView(NativeReviewActivity.this);
									if (r % 2 == 0) {
		...
		case Cnt.TYPE_DROP_DOWN_LIST:// 下来列表
			...
			if (!Util.isEmpty(columns)) {
				ArrayAdapter<String> aa = new ArrayAdapter<String>(NativeReviewActivity.this,
		...
		case Cnt.TYPE_FREE_TEXT_BOX:// 单行文本框
			...
			if (isNew) {
				...
				LinearLayout tl = new LinearLayout(NativeReviewActivity.this);
				...
				for (int i = 0; i < tbRows; i++) {
					LinearLayout tbr = new LinearLayout(NativeReviewActivity.this);// 每行的TableRow对象
					...
					for (int col = 0; col < q.freeTextColumn; col++) {
						...
						LinearLayout itemll = new LinearLayout(NativeReviewActivity.this);// 每一列的整体布局
						LinearLayout lLeft = new LinearLayout(NativeReviewActivity.this);
						LinearLayout lRight = new LinearLayout(NativeReviewActivity.this);
				...
				if (!Util.isEmpty(q.qCaption)) {
					...
					if (q.qCaption.contains("%%") || q.qCaption.contains("%%%%")) {
						...
						// 判断出生成几个小%%%%追加说明
						String[] tvCount = q.qCaption.split("%%");
						LinearLayout tbr = new LinearLayout(NativeReviewActivity.this);
						tbr.setOrientation(LinearLayout.HORIZONTAL);
						tbr.setGravity(Gravity.CENTER_VERTICAL);
						ArrayList<LinearLayout> CapLary = new ArrayList<LinearLayout>();// 每行的TableRow对象
						for (int col = 0; col < tvCount.length; col++) {
							LinearLayout capItem = new LinearLayout(NativeReviewActivity.this);
						...
						if (1 == q.qLinkage) {
							...
							if (!Util.isEmpty(item.leftsideWord)) {
								...
								if (i == 0) {
									...
									citySpinner = new Spinner(NativeReviewActivity.this);
									...
									countrySpinner = new Spinner(NativeReviewActivity.this);
							...
							// 维码扫描 item.scanning
							if (6 == item.type) {
								iv = new ImageView(NativeReviewActivity.this);
	...
	private void initSysMenu() {
		...
		menuListView.add(HotalkMenuView.MENU_VIEW_CONTACT,
				NativeReviewActivity.this.getString(R.string.read_voice, mp3coint + ""), R.drawable.test_zsj33);
		menuListView.add(HotalkMenuView.MENU_ADD_CONTACT,
				NativeReviewActivity.this.getString(R.string.read_vedio, mp4coint + ""), R.drawable.test_zsj33);
	}
