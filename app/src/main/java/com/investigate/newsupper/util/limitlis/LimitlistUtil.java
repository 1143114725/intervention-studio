...
public class LimitlistUtil {
	 ...
	 */
	// 判断数据列表
	static ArrayList<AshingBean> ashingBeans = new ArrayList<AshingBean>();
	 ...
	 * @return
	 */
	public static ArrayList<AshingBean> getlimlist(Question q) {
		...
		if (!Util.isEmpty(tbColumns)) {
			for (int i = 0; i < tbColumns.size(); i++) {
				...
				if (!Util.isEmpty(tbColumns.get(i).getLimitList())) {
					// 添加到数组里
					ashingBeans = inlimitList(i + "", tbColumns.get(i)
							.getLimitList());
				}
			}
		}
	 ...
	 * @return
	 */
	public static ArrayList<AshingQidBean> getlimlists(ArrayList<Question> qs) {
		...
		for (int s = 0; s < qs.size(); s++) {
			...
			qidlist.add(new AshingQidBean(q.qIndex+"", ashingBeans));
		}
	 ...
	 * @return 判断禁用的数组
	 */
	public static ArrayList<AshingBean> inlimitList(String etid, String str) {
			...
			for (int i = 0; i < mayStrs.length; i++) {
				...
				if (-1 != mayStrs[i].indexOf("&")) {
					...
					for (int j = 0; j < andStrs.length; j++) {
						...
						ashingBeans.add(new AshingBean(etid, commandStrs[0],
					...
					ashingBeans.add(new AshingBean(etid, commaStrs[0], "",
